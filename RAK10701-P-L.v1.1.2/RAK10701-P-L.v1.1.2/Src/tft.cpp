...
void refreshBAT(uint8_t date)
{
	...
	if ((batValHistory[0] == 4) && (batValHistory[1] == 4) && (batValHistory[2] == 4) && (batValHistory[3] == 4) && (batValHistory[4] == 4) && (batValHistory[5] == 4) && (batValHistory[6] == 4) && (batValHistory[7] == 4) && (batValHistory[8] == 4) && (batValHistory[9] == 4) && (changeIng == 1)) // Fully charged and USB detected.  Filter out voltage jumps during charging.
	{
...
void subHomePage()
{
	...
	sf_setCursor(DISTANCE_X, DISTANCE_Y);
	sf_writeStr("Distance", strlen("Distance"), LEFT_ALIGNED);
	sf_setCursor(DISTANCE_X, DISTANCE_Y + 17);
	sf_writeStr("MAX :", strlen("MAX :"), LEFT_ALIGNED);
	sf_setCursor(DISTANCE_X + 78, DISTANCE_Y + 17);
	sf_writeStr("m", strlen("m"), LEFT_ALIGNED);
	sf_setCursor(DISTANCE_X, DISTANCE_Y + 34);
	sf_writeStr("MIN :", strlen("MIN :"), LEFT_ALIGNED);
	sf_setCursor(DISTANCE_X + 78, DISTANCE_Y + 34);
	sf_writeStr("m", strlen("m"), LEFT_ALIGNED);

	sf_setCursor(RSSI_X, RSSI_Y);
	sf_writeStr("RSSI", strlen("RSSI"), LEFT_ALIGNED);
	sf_setCursor(RSSI_X, RSSI_Y + 17);
	sf_writeStr("MAX :", strlen("MAX :"), LEFT_ALIGNED);
	sf_setCursor(RSSI_X + 68, RSSI_Y + 17);
	sf_writeStr("dBm", strlen("dBm"), LEFT_ALIGNED);
	sf_setCursor(RSSI_X, RSSI_Y + 34);
	sf_writeStr("MIN :", strlen("MIN :"), LEFT_ALIGNED);
	sf_setCursor(RSSI_X + 68, RSSI_Y + 34);
	sf_writeStr("dBm", strlen("dBm"), LEFT_ALIGNED);

	tft.drawLine(212, 118, 307, 118, TFT_WHITE);

	sf_setCursor(SNR_X, SNR_Y);
	sf_writeStr("SNR :", strlen("SNR :"), LEFT_ALIGNED);
	sf_setCursor(SNR_X + 68, SNR_Y);
	sf_writeStr("dB", strlen("dB"), LEFT_ALIGNED);

	sf_setTextColor(TFT_WHITE, BBG_CLOR, false);
	sf_setCursor(LAT_X, LAT_Y);
	sf_writeStr("Lat", strlen("Lat"), LEFT_ALIGNED);
	sf_setCursor(LAT_X + 31, LAT_Y);
	sf_writeStr(":", strlen(":"), LEFT_ALIGNED);
	sf_setCursor(LONG_X, LONG_Y);
	sf_writeStr("Long", strlen("Long"), LEFT_ALIGNED);
	sf_setCursor(LONG_X + 31, LONG_Y);
	sf_writeStr(":", strlen(":"), LEFT_ALIGNED);
