...
void addHotspotsDate(hotspotsDate_t *date, uint16_t orDate)
{
	...
	*/
	if (orDate > 30)
		orDate = 30; // avoid over the graph.
...
void addRssiDate(rssiDate_t *date, int16_t orMaxDate, int16_t orMinDate)
{
	...
	*/
	if (orMaxDate > 20)
		orMaxDate = 20; // avoid over the graph.
	if (orMaxDate < -130)
		orMaxDate = -130; // avoid over the graph.
	if (orMinDate > 20)
		orMinDate = 20; // avoid over the graph.
	if (orMinDate < -130)
		orMinDate = -130; // avoid over the graph.
